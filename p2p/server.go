...
// Package p2p implements the PhiChain p2p network protocols.
package p2p
...
import (
	...
	"github.com/PhicLabs/phichain/p2p/discover"
	"github.com/PhicLabs/phichain/p2p/enode"
	"github.com/PhicLabs/phichain/p2p/enr"
	"github.com/PhicLabs/phichain/p2p/nat"
	"github.com/PhicLabs/phichain/p2p/netutil"
)
...
func (srv *Server) setupDiscovery() error {
	...
	if srv.NAT != nil {
		if !realaddr.IP.IsLoopback() {
			srv.loopWG.Add(1)
			go func() {
				nat.Map(srv.NAT, srv.quit, "udp", realaddr.Port, realaddr.Port, "PhiChain discovery")
				srv.loopWG.Done()
			}()
		}
	}
...
func (srv *Server) setupListening() error {
	...
	if tcp, ok := listener.Addr().(*net.TCPAddr); ok {
		...
		if !tcp.IP.IsLoopback() && srv.NAT != nil {
			srv.loopWG.Add(1)
			go func() {
				nat.Map(srv.NAT, srv.quit, "tcp", tcp.Port, tcp.Port, "PhiChain p2p")
				srv.loopWG.Done()
			}()
		}
	}
...
type NodeInfo struct {
	...
	ENR   string `json:"enr"`   // PhiChain Node Record
